...
class ParagraphTest {
    ...
    fun companionParseSuccessful(){
        val pass = Paragraph.parse(testString, root,0)
        assertEquals(pass.inline.size, 1)
        assertEquals(pass.inline[0]::class, InlineString::class)
        assertEquals((Paragraph.parse(testString, root, 0).inline[0] as InlineString).line, testString)
    }

    @Test(expected = AssertionError::class)
    fun companionParseFail(){
        Paragraph.parse(testBlank, root, 0)
    }

    @Test
    fun match() {
        val paragraph = Paragraph.parse(testString, root,0)
